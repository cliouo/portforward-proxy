...
This is a flexible port forwarding tool written in Go that supports both command-line and configuration file modes. It can forward TCP connections from local ports to remote addresses, optionally through HTTP proxies.
...
# Run with command-line arguments (single forward)
./portforward.exe -local <port> -remote <address> -rport <port> [-proxy <proxy_url>]

# Run with configuration file (multiple forwards)
./portforward.exe -c config.toml

# Build with Go modules
go mod tidy
go build

# Test build for multiple platforms (via goreleaser)
...
**main.go** - Single-file architecture containing all functionality:

1. **Configuration System** (lines 17-28)
   - `Config` and `ForwardConfig` structs define TOML-based configuration
   - Supports both command-line flags and config file modes (mutually exclusive)

2. **Dual-Mode Operation** (lines 30-61)
   - Command-line mode: Single forward using `-local`, `-remote`, `-rport`, `-proxy` flags
   - Config file mode: Multiple forwards via TOML file (default: `config.toml`)
   - Cannot use both modes simultaneously

3. **Connection Handling** (lines 63-122)
   - `handleSingleForward()` and `handleForward()`: TCP listener setup
   - `handleConnection()`: Bidirectional data relay using `io.Copy()` in goroutines
   - Each forward runs in its own goroutine with dedicated logger

4. **HTTP Proxy Support** (lines 124-158)
   - `dialThroughProxy()`: Implements HTTP CONNECT tunnel method
   - Establishes proxy connection, sends CONNECT request, validates 200 response

### Configuration File Format

Uses TOML with `[[forward]]` array entries. Each entry has:
- `local`: Local listening port
- `remote`: Target hostname/IP
- `rport`: Target port
- `proxy`: HTTP proxy URL (optional)
- `status`: Boolean to enable/disable this forward

See `config.toml.example` for reference.

### Logging

Each forward group gets a prefixed logger (`[转发 N]` for config mode, `[单一转发]` for CLI mode) for clear multi-forward tracking.

## Key Implementation Details

- Uses goroutines for concurrent connection handling and bidirectional data transfer
- HTTP proxy uses CONNECT method for TCP tunneling
- Main goroutine blocks on `select {}` to keep multi-forward mode alive
- 30-second connection timeout for remote/proxy connections
- Error handling preserves connection state and logs issues without crashing

## Release Process

This project uses GoReleaser with GitHub Actions (`.github/workflows/release.yaml`). Builds are configured in `.goreleaser.yaml` for:
